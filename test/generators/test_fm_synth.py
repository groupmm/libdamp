...
class TestFMSynth:
    def test_requires_update_before_generate(self):
        fm = FMSynth(N=64, fs=16000.0, P=1, connections=[])
    ...
    def test_single_unconnected_operator_is_a_pure_sine(self):
        ...
        N = 256
        f0 = 220.0
        fm = FMSynth(N=N, fs=fs, P=1, connections=[])
    ...
    def test_phase_is_continuous_across_successive_generate_calls(self):
        ...
        fm = FMSynth(N=N, fs=fs, P=2, connections=[(1, 0)])
        ...
        fm_long = FMSynth(N=N, fs=fs, P=2, connections=[(1, 0)])
