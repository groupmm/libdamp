...
class TestWeightedTableOsc:
    def test_output_shape(self):
        ...
        osc = WeightedTableOsc(frame_len=32, table=table, fs=16000.0, mode="wave")
    ...
    def test_only_selected_entry_weight_matches_table_osc_single_entry(self):
        table = _sine_table(L=64, M=2)
        osc = WeightedTableOsc(frame_len=32, table=table, fs=16000.0, mode="wave", table_freq=16000.0 / 64)
    ...
    def test_unknown_mode_raises(self):
        ...
        with pytest.raises(ValueError, match="Unknown TableOsc mode"):
            WeightedTableOsc(frame_len=32, table=table, fs=16000.0, mode="bogus")

    def test_mismatched_f0_and_weighting_shapes_raise(self):
        table = _sine_table()
        osc = WeightedTableOsc(frame_len=32, table=table, fs=16000.0)
