...
class TestSimpleFilteredNoise:
    def test_output_shape(self):
        gen = SimpleFilteredNoise(L=128, N=64, fs=16000.0, freq_bands=[100.0, 1000.0, 4000.0])
    ...
    def test_update_and_clear_are_safe_no_ops(self):
        gen = SimpleFilteredNoise(L=64, N=32, fs=16000.0, freq_bands=[100.0, 1000.0])
        gen.update()
        gen.clear()
    ...
    def test_zero_magnitude_produces_silence(self):
        torch.manual_seed(0)
        gen = SimpleFilteredNoise(L=128, N=64, fs=16000.0, freq_bands=[100.0, 1000.0])
