...
class TestTableOsc:
    def test_output_shape(self):
        ...
        osc = TableOsc(N=32, table=table, table_param=torch.tensor([0.0, 1.0]), fs=16000.0, mode="wave")
    ...
    def test_pulse_mode_period_matches_fs_over_f0(self):
        ...
        osc = TableOsc(N=L * 4, table=table, table_param=torch.tensor([0.0]), fs=fs, mode="pulse", interp_entry=False)
    ...
    def test_unknown_mode_raises(self):
        ...
        with pytest.raises(ValueError, match="Unknown TableOsc mode"):
            TableOsc(N=32, table=table, table_param=torch.tensor([0.0, 1.0]), fs=16000.0, mode="bogus")

    def test_unknown_normalization_raises(self):
        table = _sine_table()
        with pytest.raises(ValueError, match="Unknown normalization method"):
            TableOsc(N=32, table=table, table_param=torch.tensor([0.0, 1.0]), fs=16000.0, normalize="bogus")

    def test_peak_normalization_scales_table_to_unit_peak(self):
        table = torch.stack([torch.full((64,), 0.5), torch.full((64,), 2.0)])
        osc = TableOsc(N=32, table=table, table_param=torch.tensor([0.0, 1.0]), fs=16000.0, normalize="peak")
    ...
    def test_mismatched_f0_and_table_select_shapes_raise(self):
        table = _sine_table()
        osc = TableOsc(N=32, table=table, table_param=torch.tensor([0.0, 1.0]), fs=16000.0)
