...
class TestSignalF0ExcerptsDataset:
    ...
    num_frames = 8
    SAMPLES = 8192  # divisible by num_frames, long enough for n_fft=4096 with center padding
    ...
    def _write_signal_f0(self, tmp_path, n=2, samples=SAMPLES, f0_hz=200.0):
        ...
        f0 = np.full((n, self.num_frames), f0_hz, dtype=np.float32)
        _write_npy(tmp_path, "signal", "train", signal)
        _write_npy(tmp_path, "f0", "train", f0)
        return signal, f0
    ...
    def test_default_return_keys_are_signal_f_a(self, tmp_path):
        ...
        ds = SignalF0ExcerptsDataset(str(tmp_path), num_harm=3, num_frames=self.num_frames, fs=16000.0, ds_split="train")
        signal, f, a = ds[0]
        assert signal.shape == (1, self.SAMPLES)
        # "f"/"a" have shape (harmonics, frames) = (M, F)
        assert f.shape == (3, self.num_frames)
        assert a.shape == (3, self.num_frames)
    ...
    def test_amplitudes_above_nyquist_are_zeroed_but_frequencies_are_kept(self, tmp_path):
        ...
        ds = SignalF0ExcerptsDataset(str(tmp_path), num_harm=2, num_frames=self.num_frames, fs=16000.0, ds_split="train")
        _, f, a = ds[0]
    ...
    def test_extra_return_key_is_loaded_and_appended(self, tmp_path):
        ...
        ds = SignalF0ExcerptsDataset(str(tmp_path), num_harm=2, num_frames=self.num_frames, fs=16000.0, ds_split="train", return_keys=["f0", "label"])
    ...
    def test_signal_length_not_divisible_by_F_raises(self, tmp_path):
        ...
        f0 = np.zeros((1, self.num_frames), dtype=np.float32)
        _write_npy(tmp_path, "signal", "train", signal)
        _write_npy(tmp_path, "f0", "train", f0)
